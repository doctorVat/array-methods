...
// const a = users.filter(({ip_address}) => ip_address.split('.')[0] < 128)
// console.log(a);
...
// const b = users.findIndex(({ip_address}) => {
//   const f = ip_address.split('.')[0];
//   return f>=128 && f<192 
// })
...
// // const f = users.find(({email})=> email.includes('soundcloud'))
...
//  const f = users.fill('deleted')//заменяет все элементы на deleted
//  console.log(f);
...
*/
const arra = accounts[0].tags.flat()// возвращает новый массив, в котором все элементы вложенных подмассивов были рекурсивно "подняты" на указанный
// console.log(arra);
...
*/
const a = accounts[0].tags.flat().includes('consectetur')
// console.log(a);
...
*/
const b = accounts.map(({tags}) => tags.join(',')).join(',')
// console.log(b);
...
*/
// const ar = accounts.map(({friends}) => friends.map(({name}) => name)).flat()
...
*/
const ar = accounts.filter(({balance, age}) => age <=30 && parseInt(balance.replace(/\D/g,'')) > 1500)
              .map(({friends}) => friends.map(({name}) => name))
              
              .flat()

        // parseInt(balance.replace(/\D/g,''))
